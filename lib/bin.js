#!/usr/bin/env node
import { D as workbuddyOwnAuthPath, a as readHostHeartbeat, c as WORKBUDDY_CONNECT_BUILD, g as resolveProductConfigPath, h as readProductConfigModels, l as WORKBUDDY_CONNECT_VERSION, o as workbuddyHostHeartbeatPath, r as isHeartbeatProcessAlive, u as WorkBuddyUpstreamClient, x as WorkBuddyCredentialStore } from "./host-heartbeat-B-ko2SwT.js";
...
* up empty.
*/
async function inspectModelCatalog() {
	const path = resolveProductConfigPath();
	try {
		return {
			path,
			models: (await readProductConfigModels(path)).length
		};
	} catch (error) {
		return {
			path,
...
async function doctor(jsonOutput) {
		...
		*/
		...
		hostHeartbeat: {
			...
			...heartbeat === void 0 ? {} : {
				...
				pid: heartbeat.pid,
				pluginBuild: heartbeat.pluginBuild
			},
			processAlive: hostAlive
		},
		modelCatalog,
		signIn: status.state,
		...status.source === void 0 ? {} : { signInSource: status.source },
		...status.reason === void 0 ? {} : { signInReason: status.reason },
		hints: [
			...status.reason !== void 0 ? [`Stored sign-in is unusable: ${status.reason}`] : status.state === "signed-in" ? [] : ["Sign in once in the WorkBuddy desktop app, then run status again."],
			...
			...modelCatalog.error === void 0 ? [] : [`Model catalog is empty: ${modelCatalog.error}. Override the path with ACC_PRODUCT_CONFIG_PATH if the cache lives elsewhere.`]
		]
	};
	if (jsonOutput) printJson(report);
		...
		`Host bundle: ${hostAlive ? `running (pid ${heartbeat.pid}, build ${heartbeat.pluginBuild})` : heartbeat !== void 0 ? "stale heartbeat (process exited)" : `not started (CLI build ${WORKBUDDY_CONNECT_BUILD})`}`,
		`Sign-in state: ${report.signIn}${status.reason === void 0 ? "" : ` (${status.reason})`}`,
		...status.source === "desktop-unlocked" ? ["Sign-in source: desktop encrypted store (unlocked locally on this machine)"] : [],
		modelCatalog.error === void 0 ? `Model catalog: ${modelCatalog.models} models from ${modelCatalog.path}` : `Model catalog: unavailable (${modelCatalog.error})`,
...
async function status(jsonOutput) {
	...
	if (authStatus.state !== "signed-in") {
		if (jsonOutput) printJson({
			...
			build: WORKBUDDY_CONNECT_BUILD,
			status: "signed-out",
			...authStatus.reason === void 0 ? {} : { signInReason: authStatus.reason },
			hostBundle: hostState,
			...heartbeat === void 0 ? {} : { hostBuild: heartbeat.pluginBuild }
		});
	...
	process.stdout.write([
		...
		...authStatus.source === "desktop-unlocked" ? ["Sign-in source: read from the desktop app's encrypted store (unlocked locally on this machine)"] : [],
