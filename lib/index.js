import { A as defaultAppBinaryCandidates, C as defaultDesktopAuthPath, D as WORKBUDDY_APP_BINARY_ENV, E as workbuddyOwnAuthPath, F as openSealedFields, I as resetProtectorKeyCache, L as resolveProtectorKey, M as deriveProtectorKey, N as envelopeAad, O as WorkBuddyAtRestError, P as isSealedField, S as defaultDesktopAuthCandidates, T as parseWorkBuddyAuth, _ as WORKBUDDY_AUTH_FILE_ENV, a as readHostHeartbeat, b as WorkBuddyCredentialStore, d as normalizeCredits, f as prepareChatBody, g as WORKBUDDY_AUTH_FILENAME, i as processStartTimeMs, j as deriveKeyId, k as createAtRestUnlocker, l as WorkBuddyUpstreamClient, n as clearHostHeartbeat, o as workbuddyHostHeartbeatPath, p as regionOf, r as isHeartbeatProcessAlive, s as writeHostHeartbeat, t as WORKBUDDY_HOST_HEARTBEAT_FILENAME, u as classifyUpstreamError, v as WORKBUDDY_CREDENTIAL_UNREADABLE_CODE, w as inspectWorkBuddyAuthDocument, x as WorkBuddyCredentialUnreadableError, y as WORKBUDDY_ENCRYPTED_AT_REST_REASON } from "./host-heartbeat-LNI7F9JC.js";
...
function preferredDefaultEffort(efforts) {
	const tiers = efforts.filter((effort) => effort.id !== "off");
	if (tiers.length === 0) return void 0;
	return (tiers.length === 1 ? tiers[0] : tiers[tiers.length - 2])?.id;
}
/**
...
*/
function withPreferredEffort(resolved) {
	const reasoning = resolved.reasoning;
	if (reasoning === void 0) return resolved;
	const preferred = preferredDefaultEffort(reasoning.efforts);
	if (preferred === void 0 || preferred === reasoning.defaultEffort) return resolved;
	return {
		...resolved,
		reasoning: {
			...reasoning,
...
var WorkBuddyPiAiAdapter = class extends PiAiAdapter {
	...
	async resolveModel(provider, model, signal) {
		const withEffort = withPreferredEffort(await super.resolveModel(provider, model, signal));
		const info = this.infoFor(model);
		if (info === void 0) return withEffort;
		const promo = promoDescription(info);
		return {
			...withEffort,
...
async function workBuddyWebStatus(deps) {
	...
	if (authStatus.state !== "signed-in") return authStatus.reason === void 0 ? { status: "signed-out" } : {
		status: "signed-out",
...
*/
function registerWorkBuddyRefreshRoute(ctx, deps) {
	ctx.effect(() => {
		const dispose = ctx.webServer.register({
			kind: "exact",
...
function apply(ctx, config) {
	...
	*/
	async function refreshCatalog() {
		...
		if (handle === void 0) throw new Error("the WorkBuddy provider is not registered yet; the loopback endpoint is still starting");
	...
	shim.ready.then(async () => {
		...
		try {
			...
			ctx.logger.error("dsh-workbuddy-connect: model catalog unavailable (local product-config cache read failed); serving no models", error);
		}
		if (stopped) return;
		try {
...
//#endregion
export { Config, FALLBACK_WORKBUDDY_MODELS, WORKBUDDY_APP_BINARY_ENV, WORKBUDDY_AUTH_FILENAME, WORKBUDDY_AUTH_FILE_ENV, WORKBUDDY_CREDENTIAL_UNREADABLE_CODE, WORKBUDDY_ENCRYPTED_AT_REST_REASON, WORKBUDDY_HOST_HEARTBEAT_FILENAME, WORKBUDDY_PROVIDER, WORKBUDDY_SETTINGS_NS, WORKBUDDY_STREAM_IDLE_TIMEOUT_MS, WorkBuddyAtRestError, WorkBuddyCatalog, WorkBuddyCredentialStore, WorkBuddyCredentialUnreadableError, WorkBuddyUpstreamClient, apply, classifyUpstreamError, clearHostHeartbeat, createAtRestUnlocker, createWorkBuddyAdapter, createWorkBuddyShim, defaultAppBinaryCandidates, defaultDesktopAuthCandidates, defaultDesktopAuthPath, deriveKeyId, deriveProtectorKey, envelopeAad, inject, inspectWorkBuddyAuthDocument, isSealedField as isEncryptedFieldWrapper, isSealedField, isHeartbeatProcessAlive, name, normalizeCredits, openSealedFields, parseWorkBuddyAuth, prepareChatBody, processStartTimeMs, readHostHeartbeat, regionOf, resetProtectorKeyCache, resolveProtectorKey, workbuddyHostHeartbeatPath, workbuddyOwnAuthPath };
