...
function toPiModel(info, baseUrl) {
	...
	if (reasoningCfg !== void 0 && Array.isArray(reasoningCfg["supportedEfforts"]) && reasoningCfg["supportedEfforts"].length > 0) {
		const supported = reasoningCfg["supportedEfforts"];
		thinkingLevelMap = {};
		for (const level of THINKING_LEVELS) thinkingLevelMap[level] = supported.includes(level) ? level : null;
	}
	return {
		id: info.id,
		name: info.name,
