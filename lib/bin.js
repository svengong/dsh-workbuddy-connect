#!/usr/bin/env node
import { C as workbuddyOwnAuthPath, a as readHostHeartbeat, c as WORKBUDDY_CONNECT_VERSION, h as FALLBACK_WORKBUDDY_MODELS, l as WorkBuddyUpstreamClient, m as resolveProductConfigPath, o as workbuddyHostHeartbeatPath, p as readProductConfigModels, r as isHeartbeatProcessAlive, y as WorkBuddyCredentialStore } from "./host-heartbeat-XTrX-nPb.js";
...
async function doctor(jsonOutput) {
	...
	const report = {
		...
		hints: [
			...
			...modelCatalog.error === void 0 ? [] : [`Model catalog falls back to ${FALLBACK_WORKBUDDY_MODELS.length} static models: ${modelCatalog.error}. Override the path with ACC_PRODUCT_CONFIG_PATH if the cache lives elsewhere.`]
		]
	};
