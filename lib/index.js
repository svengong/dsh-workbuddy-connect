import { C as workbuddyOwnAuthPath, S as parseWorkBuddyAuth, _ as WORKBUDDY_AUTH_FILENAME, a as readHostHeartbeat, b as defaultDesktopAuthCandidates, d as prepareChatBody, f as regionOf, g as WorkBuddyCatalog, h as FALLBACK_WORKBUDDY_MODELS, i as processStartTimeMs, l as WorkBuddyUpstreamClient, n as clearHostHeartbeat, o as workbuddyHostHeartbeatPath, r as isHeartbeatProcessAlive, s as writeHostHeartbeat, t as WORKBUDDY_HOST_HEARTBEAT_FILENAME, u as classifyUpstreamError, v as WORKBUDDY_AUTH_FILE_ENV, x as defaultDesktopAuthPath, y as WorkBuddyCredentialStore } from "./host-heartbeat-XTrX-nPb.js";
...
function toPiModel(info, baseUrl) {
	...
	const supportsReasoning = info.supportsReasoning === true && (reasoningCfg !== void 0 || builtin !== void 0);
...
function apply(ctx, config) {
	...
	shim.ready.then(() => {
		...
		(async () => {
			try {
				if (await store.current() === void 0 || stopped) return;
				const models = await client.fetchModels();
				if (stopped) return;
				catalog.set([...models]);
				invalidate?.();
			} catch (error) {
				ctx.logger.warn("dsh-workbuddy-connect: local model catalog cache unavailable; serving the static fallback list", error);
			}
		})();
	}).catch((error) => {
