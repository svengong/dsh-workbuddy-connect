...
| 手段 | 生效范围 | 代价 |
|---|---|---|
| 点刷新按钮（v0.4.2） | 只重读缓存 + 重发路由 | 一键 |
| 插件面板里把开关关掉再打开 | 重新 `apply()` 整个插件 | 会写 `cordis.patch.yml`；依赖 HMR，没开 HMR 的 profile 会提示需要重启 |
| 重启 DSH | 全部重新加载 | 最彻底最重 |

**只改客户端包时不需要重启**：DSH 的 client-modules 会在插件包变化后重新下发 bundle（boot payload 里的 `rev` 会变），刷新页面即可。**改了 Host 侧代码才需要重启或重载插件。**
