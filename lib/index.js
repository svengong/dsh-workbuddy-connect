...
function toPiModel(info, baseUrl) {
	...
	let supported;
	if (reasoningCfg !== void 0) {
		const explicit = reasoningCfg["supportedEfforts"];
		if (Array.isArray(explicit) && explicit.length > 0) supported = explicit;
		else if (typeof reasoningCfg["effort"] === "string" && reasoningCfg["effort"] !== "") supported = [reasoningCfg["effort"]];
	}
	if (supported !== void 0 && supported.length > 0) {
		thinkingLevelMap = {};
