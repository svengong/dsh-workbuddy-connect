...
var WorkBuddyUpstreamClient = class {
	...
	*/
	async fetchModels(credential) {
		...
		if (credential === void 0) throw new Error(`workbuddy model catalog unavailable: local product-config cache failed (${String(cacheError)}); no credential for the network catalog — open the WorkBuddy desktop app once so it refreshes`);
		try {
			return await this.fetchModelsFromNetwork(credential);
		} catch (error) {
			throw new Error(`workbuddy model catalog unavailable: local product-config cache failed (${String(cacheError)}); network catalog failed (${String(error)})`);
		}
	}
	/** GET the personal model catalog and keep the `cli` agent's models only. */
	async fetchModelsFromNetwork(credential) {
...
//#endregion
export { parseWorkBuddyAuth as C, defaultDesktopAuthPath as S, WorkBuddyCatalog as _, readHostHeartbeat as a, WorkBuddyCredentialStore as b, WORKBUDDY_CONNECT_VERSION as c, normalizeCredits as d, prepareChatBody as f, FALLBACK_WORKBUDDY_MODELS as g, resolveProductConfigPath as h, processStartTimeMs as i, WorkBuddyUpstreamClient as l, readProductConfigModels as m, clearHostHeartbeat as n, workbuddyHostHeartbeatPath as o, regionOf as p, isHeartbeatProcessAlive as r, writeHostHeartbeat as s, WORKBUDDY_HOST_HEARTBEAT_FILENAME as t, classifyUpstreamError as u, WORKBUDDY_AUTH_FILENAME as v, workbuddyOwnAuthPath as w, defaultDesktopAuthCandidates as x, WORKBUDDY_AUTH_FILE_ENV as y };
